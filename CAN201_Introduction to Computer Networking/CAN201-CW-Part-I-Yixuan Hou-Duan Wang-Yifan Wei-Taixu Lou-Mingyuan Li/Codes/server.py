...
def data_process(username, request_operation, json_data, connection_socket):
    ...
    if request_operation == OP_SAVE:
        ...
        try:
            with open(join('data', username, key), 'w') as fid:
                ...
                logger.info(f'<-- Data is saved with key "{key}"')   # bug2
                connection_socket.send(
    ...
    if request_operation == OP_DELETE:
        ...
        try:
            ...
            logger.info(f'<-- The "key" {json_data[FIELD_KEY]} is deleted.')   # bug3
            connection_socket.send(
...
def file_process(username, request_operation, json_data, bin_data, connection_socket):
    ...
    if request_operation == OP_SAVE:
        ...
        if FIELD_SIZE not in json_data.keys():
            ...
            connection_socket.send(
                make_response_packet(OP_SAVE, 410, TYPE_FILE, f'This file "size" has to be included', {}))   # bug4
            return
        ...
        try:
            ...
            logger.info(f'<-- Upload plan: key {key}, total block number {total_block}, block size {block_size}.')  # bug5
            connection_socket.send(
    ...
    if request_operation == OP_DELETE:
        ...
        try:
            ...
            logger.info(f'<-- The "key" {json_data[FIELD_KEY]} is deleted.')   # bug6
            connection_socket.send(
    ...
    if request_operation == OP_UPLOAD:
        ...
        if os.path.exists(join('file', username, json_data[FIELD_KEY])) is True:
            ...
            connection_socket.send(
                make_response_packet(OP_UPLOAD, 402, TYPE_FILE, f'The "key" {json_data[FIELD_KEY]} is completely uploaded.', {}))  # bug7
            return
        ...
        if os.path.exists(join('tmp', username, json_data[FIELD_KEY])) is False:
            ...
            connection_socket.send(
                make_response_packet(OP_UPLOAD, 404, TYPE_FILE,    # bug8
                                     f'The "key" {json_data[FIELD_KEY]} is not accepted for uploading.',
                                     {}))
            return
        ...
        if block_index >= total_block:
            ...
            connection_socket.send(
                make_response_packet(OP_UPLOAD, 405, TYPE_FILE, f'The "block_index" exceed the max index.', {}))   # bug9
            return
        if block_index < 0:
            logger.error(f'<-- The "block_index" should >= 0.')
            connection_socket.send(
                make_response_packet(OP_UPLOAD, 406, TYPE_FILE, f'The "block_index" should >= 0.', {}))  # bug10
            return
    ...
    if request_operation == OP_DOWNLOAD:
        ...
        if block_index >= total_block:
            logger.error(f'<-- The "block_index" exceed the max index.')
            connection_socket.send(
                make_response_packet(OP_GET, 406, TYPE_FILE, f'The "block_index" exceed the max index.', {}))  # bug11
            return
        if block_index < 0:
            logger.error(f'<-- The "block_index" should >= 0.')
            connection_socket.send(
                make_response_packet(OP_GET, 406, TYPE_FILE, f'The "block_index" should >= 0.', {}))  # bug12
            return
    ...
    while True:
        ...
        if request_operation == OP_LOGIN:
            if request_type != TYPE_AUTH:
                ...
                if hashlib.md5(json_data[FIELD_USERNAME].encode()).hexdigest().lower() != json_data[FIELD_PASSWORD].lower():  # bug13
                    connection_socket.send(
        ...
        if request_type == TYPE_FILE:
            file_process(username, request_operation, json_data, bin_data, connection_socket)  # bug14
            continue
...
def tcp_listener(server_ip, server_port):
    ...
    """
    global logger
    server_socket = socket(AF_INET, SOCK_STREAM)  # bug15
    server_socket.setsockopt(SOL_SOCKET, SO_REUSEADDR, 1)
    server_socket.bind((server_ip, int(server_port)))
    server_socket.listen(20)
    logger.info('Server is ready')  # bug16
    logger.info(
    ...
    while True:
        try:
            ...
            th = Thread(target=Step_service, args=(connection_socket, addr))   # bug17
            th.daemon = True
            th.start()   # bug18
